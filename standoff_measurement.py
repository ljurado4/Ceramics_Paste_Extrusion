...
def image_contours(morph, height):
    ...
    cv2.drawContours(img_contours[int(height * 0.5):], contours, -1, (255, 0, 0), 2)
...
def nozzle_contour(contours, width, height):
    ...
    for contour in contours:
        ...
        if area > 200:  # Adjust threshold to focus on significant contours only
            #x is top left, y is vertical top left, w is width, h is right
            x, y, w, h = cv2.boundingRect(contour)
            if y < height * 0.2 and x<0.75*width and x>0.25*width:  # Should be closer to the top of ROI
                nozzle_bottom = (x + w // 2, int(height * 0.5) + y + h)
                nozzle_rect = (x, int(height * 0.5) + y, w, h)
    
    calibration_factor = nozzle_OD / nozzle_rect[2]  # mm per pixel
    ...
    if nozzle_rect is not None:
        cv2.rectangle(img, (nozzle_rect[0], nozzle_rect[1]), 
                        ...
                        (255, 255, 0), 2)  #cyan rectangle for nozzle
    else:
...
def plate_contour(contours):
    ...
    for contour in contours:
        area = cv2.contourArea(contour)
        if area > 1000:  # Adjust threshold for a prominent platform
            x, y, w, h = cv2.boundingRect(contour)

            ####################
            y_coords = [point[0][1] for point in contour]
            sorted_y_coords = sorted(y_coords)
            # print("sorted_y_coords", sorted_y_coords)
            ...
            # print("filtered_y_coords", filtered_y_coords)
            
            try:
                    #calculate mode of y-coordinates
    ...
    if plate_rect is not None:
        cv2.rectangle(img, (plate_rect[0], plate_rect[1]), 
                        ...
                        (255, 0, 255), 2)  #magenta rectangle for platform
    else:
...
def measure_standoff(nozzle_bottom, calibration_factor, plate_top):
    ...
    try:
        ...
        output_path = "measured_image.png"
